...
> Ветка: `main`
> Статус: `STABLE`
> Использование: `production / staging`
> Официальный install-source: `main`

`main` — это стабильная базовая ветка проекта. Именно она считается официальным install-source форка и должна использоваться для обычной установки, повторных установок, operational-сценариев и рабочего production/staging-контура.
...
- Это официальный стабильный baseline проекта.
- Эта ветка считается production-ready install-source для подтверждённого `classic`-контура.
- Все install, reset, verify, websub, backup и restore сценарии ориентируются именно на эту ветку как на базовую точку.
- Если нужна новая разработка платформенного уровня, использовать нужно не `main`, а `codex/platform-v2`.
- Branch-specific статус и границы этой ветки зафиксированы в `docs/BRANCH_CONTEXT.md`.
...
- `docs/BRANCH_CONTEXT.md` — branch-specific статус, допустимые действия и ограничения именно ветки `main`
