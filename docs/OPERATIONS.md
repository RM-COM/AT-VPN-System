...
- [2026-04-13 05:10:00] `stage=verify` теперь таким же способом валидирует и `REALITY`, а `XHTTP` self-test больше не зависит от внешнего `https://www.gstatic.com/generate_204`: probe идёт на локальный HTTPS target проекта, так что verify отражает сам baseline, а не состояние стороннего egress.
