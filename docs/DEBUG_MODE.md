...
- [2026-04-05 08:02:47] `stage=reset`, последующий clean install, `stage=verify` и `stage=websub` уже подтверждены на реальном staging VPS; следующий отдельный regression-блок — `uninstall`, `backup`, `restore`.
