...
- Активный `XHTTP` loopback self-test в `verify` для `stealth-xhttp`, чтобы transport проверялся не только по наличию socket/path
...
- `commands/xhttp-selftest.json`, `commands/xhttp-selftest.xray.log`, `commands/xhttp-selftest.curl.log` при `stealth-xhttp`
