...
- [2026-04-10 22:53:58] Текущая ближайшая задача: идти по `PROTOCOL_HARDENING_PLAN.md`, закрыть `Block C1` из `XRAY_DPI_PLAN.md` и учитывать co-tenancy риск при будущей интеграции `AWG`; затем зафиксировать production-рекомендацию в формате «основной stealth-профиль + fallback-профиль».
